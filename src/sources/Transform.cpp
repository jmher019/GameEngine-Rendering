...
bool Transform::operator==(const Transform& other) noexcept {
    return translationAndRotation == other.translationAndRotation && scale == other.scale;
}
...
Transform puggo::generalRotate(const Transform& transform, const float& degrees, const vec3& axis) noexcept {
    const fdualquat& dualQuat = transform.getTranslationAndRotation();
    const fquat rotation = glm::rotate(fquat(1.f, 0.f, 0.f, 0.f), radians(degrees), axis);
    return Transform(
        fdualquat(rotation * dualQuat.real, dualQuat.dual),
        transform.getScale()
    );
}
...
Transform puggo::generalOrbit(const Transform& transform, const float& degrees, const vec3& axis) noexcept {
    const fdualquat rotation = fdualquat(glm::rotate(fquat(1.f, 0.f, 0.f, 0.f), radians(degrees), axis), vec3(0.f, 0.f, 0.f));
    return Transform(
        rotation * transform.getTranslationAndRotation(),
        transform.getScale()
    );
}
